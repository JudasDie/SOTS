...
class Video(object):
    ...
    def __getitem__(self, idx):
        if self.imgs is None:
            return cv2.imread(self.img_names[idx]), self.gt_traj[idx], self.phrase
        else:
            return self.imgs[idx], self.gt_traj[idx], self.phrase

    def __iter__(self):
        for i in range(len(self.img_names)):
            if self.imgs is not None:
                yield self.imgs[i], self.gt_traj[i], self.phrase
            else:
                # print(self.img_names[i])
                yield cv2.imread(self.img_names[i]), self.gt_traj[i], self.phrase
