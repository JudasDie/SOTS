...
from core.mot.general import xyxy2xywh, xywh2xyxy, torch_distributed_zero_first
...
class LoadImages_panda:  # for inference
    ...
    def __next__(self):
        ...
        if int(img_path.split("_")[-1].split(".")[0]) in self.label_dict.keys():
            img0_list, start_list, split_size_list,boundary_list,index_sum_list = self.split_img(img0, output_size_list=self.split_size,
                                                                    over_lap=self.over_lap,label_list=self.label_dict[int(img_path.split("_")[-1].split(".")[0])])
        else:
            ...
            for i in range(num_i):
                ...
                for j in range(num_j):
                    ...
                    for label_i in range(len(label_list)):
                        if (x_now < label_list[label_i][1] < x_now + output_size[0] and y_now < label_list[label_i][2] < y_now + output_size[1]) or (x_now < label_list[label_i][1] + label_list[label_i][3] < x_now + output_size[0] and y_now < label_list[label_i][2] + label_list[label_i][4] < y_now + output_size[1]):
                            x_list = sorted([label_list[label_i][1]  - x_now, label_list[label_i][1] + label_list[label_i][3] - x_now, 0, output_size[0]])
                            y_list = sorted([label_list[label_i][2] - y_now, label_list[label_i][2] + label_list[label_i][4] - y_now, 0, output_size[1]])
                            iou = (x_list[2] - x_list[1]) * (y_list[2] - y_list[1]) / (label_list[label_i][3] * label_list[label_i][4])
                            ...
                            if output_size == output_size_list[0]:
                                if (w_c <= 0.7 and h_c <= 1) and iou > 0.2:
                                    if label_sort_list[label_i] == 0:
                                        index_list.append([label_i, int(x_c*136),int(y_c*76)])
...
class LoadImages_gt:  # for inference
    ...
    def __next__(self):
        ...
        if os.path.isfile(label_path):
            ...
            for i in range(len(labels0)):
                ...
                inds_x += [x]
                inds_y += [y]
                for i_m in range(int(labels[i][1]*76/len(img0)), int(labels[i][3]*76/len(img0))):
                    for j_m in range(int(labels[i][0]*136/len(img0[0])), int(labels[i][2]*136/len(img0[0]))):
                        if j_m <= 0: j_m = 0
    ...
    def __getitem__(self, idx):
        ...
        if os.path.isfile(label_path):
            ...
            labels0[:, 0] = labels0[:, 0] * output_w
            labels0[:, 1] = labels0[:, 1] * output_h
            inds = np.zeros((len(labels0),), dtype=np.int64)
            for i in range(len(labels0)):
                inds[i] = int(labels0[i][1]) * output_w + int(labels0[i][0])
                if inds[i] < 0 or inds[i] >= 10336:
                    x = int(labels0[i][0])
                    y = int(labels0[i][1])
                    if x <= 0: x = 1
                    if x >= output_w: x = output_w - 1
                    if y < 0: y = 0
                    if y >= output_h: y = output_h - 1
                    inds[i] = y * output_w + x

        return img_path, img, img0,self.frame_id,labels, inds
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, root, paths, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            for ds, path in paths.items():
                ...
                self.label_files_dict[ds] = [x.replace('images', 'labels_with_ids').replace('.png', '.txt').replace('.jpg', '.txt')
    ...
    def __getitem__(self, index):
        ...
        if nL:
            ...
            for box_i in range(len(labels)):
                y1 = max(int(labels[box_i][2] * 76 / img.shape[0]),0)
                y2 = min(int(labels[box_i][4] * 76 / img.shape[0]),75)
                x1 = max(int(labels[box_i][1] * 136 / img.shape[1]),0)
                x2 = min(int(labels[box_i][3] * 136 / img.shape[1]),135)
...
def letterbox(img, new_shape=(640, 640), color=(127.5, 127.5, 127.5), auto=True, scaleFill=False, scaleup=True):
