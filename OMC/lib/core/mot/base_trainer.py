...
from core.mot.torch_utils import init_torch_seeds, is_parallel
...
def build_targets(p, targets, model,opt):
    # Build targets for compute_loss(), input targets(image,class,x,y,w,h)
    if opt.recheck:
        det = model.model.module.model[-1] if is_parallel(model) else model.model.model[-1]  # Detect() module
    else:
        det = model.module.model[-1] if is_parallel(model) else model.model[-1]  # Detect() module
    na, nt = det.na, targets.shape[0]  # number of anchors, targets
...
class MotLoss(torch.nn.Module):
    def __init__(self, nID, emb_dim, opt):
        ...
        self.opt = opt
        if self.opt.recheck:
            self.hmloss = nn.MSELoss(reduce=True, size_average=True).cuda()
            self.whloss_MSE = nn.MSELoss(reduce=True, size_average=True).cuda()


    def forward(self, output, targets, densemask_T, hm_mask_T, model):
        if self.opt.recheck:
            output, hm_mask, p_siammot = output[0], output[1], output[2]
        ...
        tcls, tbox, indices, anchors, indices_id, tids = build_targets(p, targets, model, self.opt)  # targets
        ...
        if self.opt.recheck:
            hm_mask = hm_mask.squeeze(dim=1)
            hmloss = self.hmloss(hm_mask_T.float(), hm_mask.float()).unsqueeze(dim=0)

            tcls, tbox_s8, indices_s8 = build_targets_siammot(p, targets, model)
            b, gj, gi = indices_s8[0]
            ps = p_siammot[b, gj, gi]  # prediction subset corresponding to targets
            # Regression
            pxy = (ps[:, :2].sigmoid() - 0.5) * w_k
            pwh = ps[:, 2:4]
            pbox = torch.cat((pxy, pwh), 1).to(device).float()  # predicted box
            whloss = self.whloss_MSE(pbox, tbox_s8[0]).unsqueeze(dim=0)
            whloss *= h['giou'] * s
            loss = lbox + lobj + id_loss * self.s_id + dmloss + whloss + hmloss
            return loss * bs, torch.cat((whloss, hmloss, lbox, lobj, loss)).detach()
