        ...
        for i in range(num_i):
            ...
            for j in range(num_j):
                    ...
                    if (x_now < label[1]-label[3]/2 < x_now + output_size[0] and y_now < label[2]-label[4]/2 < y_now + output_size[1]) or (x_now < label[1]+label[3]/2 < x_now + output_size[0] and y_now < label[2]+label[4]/2 < y_now + output_size[1]):
                        ...
                        if not (x_c < 0 or x_c > output_size[0] or y_c < 0 or y_c > output_size[1]):
                            if output_size == output_size_list[0]:
                                if w_c <= 0.5 and h_c <= 0.5:
                                    add_list = [0,x_c,y_c,w_c,h_c]
                                    if add_list not in label_list:
                                        label_list.append(add_list)
                            else:
                ...
                if len(label_list) != 0:
                    ...
                    with open(txt_path, "w") as w_txt:
                        for label in label_list:
                            label_str = '{:d},{:.6f},{:.6f},{:.6f},{:.6f}\n'.format(
                                0, label[1], label[2], label[3], label[4])
                    ...
                    img_save = cv2.resize(img_save, (1280, 1280), interpolation=cv2.INTER_LINEAR)
...
def split_track(i):
    ...
    for r_i in range(leg):
        ...
        split_img(img=img,
                  ...
                  output_size_list=[[1500, 1500], [3000, 3000], [6000, 6000], [10000, 10000], [w_img, h_img]],
                  over_lap=0.2,
