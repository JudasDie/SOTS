...
from dataset.cstrack_panda import create_dataloader
from dataset import cstrack
...
from core.mot.base_trainer_cstrack_panda import MotLoss
...
def train(hyp, opt, device, tb_writer=None):
    ...
    if rank in [-1, 0]:
        ...
        testloader = cstrack.create_dataloader(dataset_root,test_paths, imgsz, total_batch_size, gs, opt, hyp=hyp, augment=False,
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        logger.info(('\n' + '%10s' * 9) % ('Epoch', 'gpu_mem', 'GIoU','idloss','dmloss', 'obj', 'total', 'targets', 'lr'))
        if rank in [-1, 0]:
            pbar = tqdm(pbar, total=nb)  # progress bar
        optimizer.zero_grad()
        for i, (imgs, targets, paths, _, dense_mask) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                # Loss
                loss, loss_items = mot_loss(pred, targets.to(device), dense_mask.to(device), model)  # scaled by batch_size
                if rank != -1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='../lib/dataset/mot/cfg/panda.json', help='data.json path')

    parser.add_argument('--hyp', type=str, default='../experiments/train/CSTrack.yaml', help='hyperparameters path, i.e. data/hyp.scratch.yaml')
