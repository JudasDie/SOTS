...
def iou_distance(atracks, btracks):
    ...
    if (len(atracks)>0 and isinstance(atracks[0], np.ndarray)) or (len(btracks) > 0 and isinstance(btracks[0], np.ndarray)):
        ...
        for track in atracks:
            ret = np.asarray(track.mean).copy()
            ret = ret[:4] + ret[4:]
            ret[2] = ret[2] * ret[3]
            ret[:2] -= ret[2:]/2
            ret[2:] += ret[:2]
            atlbrs_kalman.append(ret)
    _ious = ious(atlbrs, btlbrs)
    cost_matrix = 1 - _ious
    k_ious = ious(atlbrs_kalman, btlbrs)
    cost_matrix_kalman = 1 - k_ious
    return np.minimum(cost_matrix,cost_matrix_kalman)
...
def fuse_motion(kf, cost_matrix, tracks, detections, only_position=False, lambda_=0.99):
    if cost_matrix.size == 0:
        return cost_matrix
    gating_dim = 8
    gating_threshold = kalman_filter.chi2inv95[gating_dim]
