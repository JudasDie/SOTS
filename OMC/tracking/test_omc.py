...
import numpy as np
import torch
from tracker.cstrack_w_recheck import JDETracker as JDETracker_wrecheck
...
from dataset.cstrack import LoadImages_jde, LoadImages_gt
...
def eval_seq(opt, dataloader, data_type, result_filename, seq, save_dir=None, show_image=True, frame_rate=30):
    ...
    tracker = JDETracker_wrecheck(opt, frame_rate=frame_rate)
    timer = Timer()
    results = []
    frame_id = 0
    seq_num = 0
    ...
    for path, img, img0, frame_id_start in dataloader:
        ...
        # online_targets = tracker.update(blob, img0, seq, save_dir,dense_mask)
        online_tlwhs = []
        online_ids = []
...
def main(opt, data_root='/data/MOT16/train', det_root=None, seqs=('MOT16-05',), exp_name='demo',
    ...
    for seq in seqs:
        ...
        output_dir = os.path.join('..', opt.output_root, 'vis')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_ours', default=False, help='test ours')

    parser.add_argument('--val_train', default=False, help='val training set')
    parser.add_argument('--val_other', default=False, help='val other set')
    ...
    parser.add_argument('--min_shape_ratio', type=float, default=4, help='filter out tiny boxes')
    ...
    parser.add_argument('--output_root', type=str, default='runs/test', help='expected output root path')
    parser.add_argument('--name', default='mot17_val', help='save to output_root/name')

    parser.add_argument('--weights', type=str, default='../weights/CSTrack_l_person.pt', help='model.pt path(s)')
    parser.add_argument('--cfg', type=str, default='../experiments/model_set/CSTrack_s.yaml', help='model.yaml path')
    ...
    parser.add_argument('--data_dir', type=str, default='/data/lc')
    ...
    parser.add_argument('--conf_thres_siammot', type=float, default=0.3, help='object confidence threshold')
    parser.add_argument('--iousfuse_thres', type=float, default=0.5, help='iou thresh for nms')
