...
ovis_base_path = '/datassd/segmentation/OVIS'
...
def crop_video(video, crop_path, instanc_size):
    ...
    video_crop_base_path = join(crop_path, 'train', video_name)
    if not exists(video_crop_base_path): makedirs(video_crop_base_path)

    video_base_path = join(ovis_base_path, 'Images', video_name)

    for idx, info in enumerate(video['frame']):
        img_name = info['img_path']
        img_path = join(video_base_path, img_name)
        ...
        cv2.imwrite(join(video_crop_base_path, '{:06d}.{:02d}.x.jpg'.format(int(idx), info['object_id'])), x)


def main(instanc_size=511, num_threads=24):
    crop_path = '/home/zpzhang/dataset/crop/ovis/crop{:d}'.format(instanc_size)
    if not exists(crop_path): makedirs(crop_path)

    annos = json.load(open('ovis.json', 'r'))
    videos = annos[0]
    n_videos = len(videos)

    # crop_video(videos[0], crop_path, instanc_size)
