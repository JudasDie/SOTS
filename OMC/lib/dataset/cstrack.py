...
def create_dataloader(root,path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8,state="train",recheck=False, single_cls=False):
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache.
    # 设置随机数种子
    setup_seed(2020)
    with torch_distributed_zero_first(rank):
    ...
    dataloader = torch.utils.data.DataLoader(dataset,
                                             ...
                                             num_workers=nw,
                                             worker_init_fn = _init_fn(2020),
                                             shuffle=False if recheck else True,
                                             sampler=dataset.pairs_index if recheck else sampler,
                                             pin_memory=True,
                                             collate_fn=LoadImagesAndLabels.collate_fn)
    return dataloader, dataset
...
def draw_gaussian_only(heatmap, center, sigma=1):
    tmp_size = sigma * 3
